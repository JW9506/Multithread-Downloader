...
int InsertTaskInfo(TaskInfo* task_info) {
    int result = ExecuteSql(
        "insert into task_info(filename, directory, url, size, progress, "
        "status, resume_support) values($s, $s, $s, $d, $d, $d, $d)",
        task_info->filename, task_info->directory, task_info->url,
        task_info->size, task_info->progress, task_info->status,
...
void UpdateTaskInfo(TaskInfo* task_info) {
    ExecuteSql("update task_info set progress=$d, status=$d where id=$d",
               task_info->progress, task_info->status, task_info->id);
}
...
TaskInfo* FindTaskInfoById(sqlite3_int64 id) {
    ...
    if (QuerySingle(&query_single_result, "select * from task_info where id=$d",
                    id) == RESULT_OK) {
...
void TaskInfoDump(TaskInfo* task_info) {
    printf("TaskInfo(\n"
           ...
           "        create_time=%s,\n"
           ")\n",
           task_info->id, task_info->filename, task_info->directory,
           task_info->url, task_info->size, task_info->progress,
           task_info->status, task_info->resume_support,
