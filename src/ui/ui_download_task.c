...
#define CASE_STATUS(status)                                                    \
    case status: status_text = status##_TEXT; break;
    // (## syntax: connect again and expand)
...
DownloadTask* CreateDownloadTask(TaskListContext* task_list_context,
    ...
    gtk_list_store_set(task_list_context->task_store, &iterator,
                       ...
                       filesize_display_buffer, COLUMN_PROGRESS,
                       GetProgress(download_task), COLUMN_STATUS,
                       GetStatusText(download_task), COLUMN_ESTIMATE_TIME, "",
                       COLUMN_TRANSFER_TIME, "", COLUMN_CREATE_TIME,
                       task_info->create_time, COLUMN_ID, task_info->id, -1);
...
void RemoveDownloadTaskFromList(DownloadTask* download_task) {
    if (download_task->row_reference) {
        ...
        gtk_list_store_remove(
            GTK_LIST_STORE(download_task->task_list_context->task_store),
            &iterator);
        gtk_tree_path_free(path);
    ...
    if (download_task->row_reference != NULL) {
        ...
        if (download_task->task_info.status == STATUS_DOWNLOADING) {
            ...
            uint64_t update_interval =
                current_time - download_task->last_update_time;
            if (update_interval > REFRESH_INTERVAL) {
                uint64_t transfer_rate = (download_task->task_info.progress -
                                          download_task->last_progress) *
                                         1000 / update_interval;
                uint64_t estimate_time =
                    transfer_rate == 0 ? 0
                                       : (download_task->task_info.size -
        ...
        gtk_list_store_set(
            ...
            COLUMN_PROGRESS, GetProgress(download_task), COLUMN_ESTIMATE_TIME,
            estimate_time_text, COLUMN_TRANSFER_TIME, transfer_rate_text, -1);
...
void UpdateDownloadTaskWithStatus(DownloadTask* download_task, int status) {
    if (download_task->task_info.status == STATUS_REMOVED) {
        ...
        printf("Download task(%lld), status: %d -> %d\n",
